// Package logo renders an ApexCode wordmark in a stylized way.
package logo
...
// Opts are the options for rendering the ApexCode title art.
type Opts struct {
...
// Render renders the ApexCode logo. Set the argument to true to render the narrow
// version, intended for use in a sidebar.
//
// The compact argument determines whether it renders compact for the sidebar
// or wider for the main pane.
func Render(version string, compact bool, o Opts) string {
	const charm = " ApexCode™"
...
// SmallRender renders a smaller version of the ApexCode logo, suitable for
// smaller windows or sidebar usage.
func SmallRender(width int) string {
	t := styles.CurrentTheme()
	title := t.S().Base.Foreground(t.Secondary).Render("Charm™")
	title = fmt.Sprintf("%s %s", title, styles.ApplyBoldForegroundGrad("ApexCode", t.Secondary, t.Primary))
	remainingWidth := width - lipgloss.Width(title) - 1 // 1 for the space after "ApexCode"
	if remainingWidth > 0 {
...
// letterP renders the letter P in a stylized way.
...
func letterX(stretch bool) string {
	...
	_ = stretch // X doesn't stretch well
	return heredoc.Doc(`
		▀▄ ▄▀
		  █
		▄▀ ▀▄`)
