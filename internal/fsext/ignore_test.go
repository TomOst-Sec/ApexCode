...
func TestApexCodeIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .apexcodeignore file that ignores .log files
	require.NoError(t, os.WriteFile(".apexcodeignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .apexcodeignore file
	apexcodeignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".apexcodeignore"), []byte(apexcodeignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .apexcodeignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .apexcodeignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .apexcodeignore in subdir that ignores normal_nested
	subApexcodeignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".apexcodeignore"), []byte(subApexcodeignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .apexcodeignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .apexcodeignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .apexcodeignore")
